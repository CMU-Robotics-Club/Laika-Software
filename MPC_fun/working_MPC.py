...
class Simulator:
    ...
    def step(self, state, u_mpc):
        ...
        for _ in range(n_steps):
            y, q1, q2 = x[0], x[1], x[2]
            dy, dq1, dq2 = x[3], x[4], x[5]
            
            # Contact
            foot_y = y - self.cfg.l1*np.cos(q1) - self.cfg.l2*np.cos(q1+q2)
            J_hip  = self.cfg.l1*np.sin(q1) + self.cfg.l2*np.sin(q1+q2)
            J_knee = self.cfg.l2*np.sin(q1+q2)
            foot_dy = dy + J_hip*dq1 + J_knee*dq2
            
            f_ext = 0.0
            if foot_y < 0:
                f_ext = -5000 * foot_y - 50 * foot_dy 
                f_ext = max(0, f_ext)

            acc = self.f_acc(x, u_mpc, f_ext).full().flatten()
            
            x[3:] += acc * self.cfg.dt_sim
            x[:3] += x[3:] * self.cfg.dt_sim
            
            # Floor safety
            if x[0] < 0.05: x[0] = 0.05; x[3] = max(0, x[3])
